...
There is currently one global optionnal parameter: `dispatchUrl`. This parameter is used to help the browser locate the net-monitor server when the base URL (http[s]://[hostname]:[port]/[context]) of the web page that makes use of net-monitor is not the same that the base URL of the server side of net-monitor (http[s]://[installation-hostname]:[installation-port]/net-monitor).
...
-->
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTExNjY1NjMyOTldfQ==
-->