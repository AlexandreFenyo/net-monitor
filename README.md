...
`dispatchUrlWebSocket` (optional): this parameter is useful when you want to connect to the dispatch servlet the web socket from another URL than the web services. one specified with dispatchUrl, for instance because you access the  for instance to help the browser locate the dispatch servlet in the net-monitor server when the base URL (http[s]://[hostname]:[port]/[context]) of the web page that makes use of net-monitor is not the same that the base URL of the server side of net-monitor. This dispatch servlet is used to access web sockets and web services on the server.
...
-->
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTU1MTQ4ODM3Nl19
-->