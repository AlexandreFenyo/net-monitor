...
var mypackage = require('./mypackage.json');
...
gulp.task('mypackage', mypackageTask);
...
function buildTask() {
  ...
  var bundled = browserify('./src/chart.js', { standalone: 'Chart' })
    ...
    .pipe(streamify(replace('{{ version }}', mypackage.version)))
...
function mypackageTask() {
  return merge(
      // gather "regular" files landing in the mypackage root
      gulp.src([outDir + '*.js', 'LICENSE.md']),

      // since we moved the dist files one folder up (mypackage root), we need to rewrite
